...
export default function FirstLetterQuiz() {
  ...
  const [target, setTarget] = useState<EnglishLetter | null>(null);
  const [options, setOptions] = useState<EnglishLetter[]>([]);
  const [selected, setSelected] = useState<string | null>(null);
  const [isCorrect, setIsCorrect] = useState<boolean | null>(null);

  const generateQuestion = () => {
    const randomTarget = ENGLISH_LETTERS[Math.floor(Math.random() * ENGLISH_LETTERS.length)];
    ...
    <QuizContainer>
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <QuestionText>{t('english.games.quiz.question')}</QuestionText>
          <SpeakerButton
            onClick={speakWord}
